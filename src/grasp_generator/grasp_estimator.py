...
from grasp_generator.models import create_model
import numpy as np
import torch
import time
import trimesh
...
class GraspEstimator:
    ...
    def generate_and_refine_grasps(
        ...
        grasps = utils.denormalize_grasps(grasps, pc_mean)
    ...
    def prepare_pc2(self, pc):
        pc_mean = np.mean(pc, 0)

        subarrays = []

        for i in range(0, self.num_batches):
            pc_sub = utils.regularize_pc_point_count(pc, self.target_pc_size)
            pc_sub -= np.expand_dims(pc_mean, 0)
            pc_sub = np.tile(pc_sub, (self.batch_size, 1, 1))
            pc_sub = torch.from_numpy(pc_sub).float().to(self.device)
    
            subarrays.append(pc_sub)

        return subarrays, pc_mean
