...
GRIPPER_PC = np.load(
    os.path.join(ROOT_PATH, 'src', 'grasp_generator', 'gripper_models', 'panda_pc.npy'),
...
def regularize_pc_point_count(pc, npoints, use_farthest_point=False):
    ...
    if pc.shape[0] > npoints:
        ...
        if required > 0:
            index = np.random.choice(range(pc.shape[0]), size=required) #size=required
...
def get_control_point_tensor(batch_size, use_torch=True, device="cpu"):
    ...
    file_niryo_xl = 'niryo_ned_xl.npy'
    file_niryo = 'niryo_ned.npy'
    file_panda = 'panda.npy'

    control_points = np.load(
        os.path.join(ROOT_PATH, 'src', 'grasp_generator', 'gripper_control_points', file_panda)
    )[:, :3]
